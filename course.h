...
public:
    ...
    // MODIFIES: this
    // EFFECTS: adds/updates Task index of type; throw exception if fails to add
    // Task
    virtual void finishTask(const std::string &type, int index, int finish_month, int
    finish_day) = 0;
    // REQUIRES: Task index of type exists in tasks. finish_month and finish_day are
    // in normal range.
    // MODIFIES: this
